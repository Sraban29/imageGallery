    ...
    <main class="container">
     <form action="" method="post" class="archive-form">
        ...
        <div class="form-group">
            <select name="title-archive" id="title-archive" class="input-text input-select">
     ...
     <form action="" method="post" class="backup-form">
        <legend class="head-2">Backup your Title</legend>
        <div class="form-group">
            <select name="title-backup" id="title-backup" class="input-text input-select">
                <option value="">SELECT TITLE</option>
                <?php
                    // echo "<option value='".$_POST['title-list']."' selected></option>";
                    $fetchTitle = "SELECT title FROM mastertab WHERE status = 0";
                    $res = mysqli_query($db, $fetchTitle);
                    if($res){
                        while($row = mysqli_fetch_array($res)){
                            echo "<option value='".$row['title']."'>".$row['title']."</option>";
            ...
            <input type="submit" name="backup-event" value="Backup" class="btn" id="backup" />
        </div>
            <?php echo $backup_msg; ?>
     </form>
 </main>
